<?php
include("admin/include/configpdo.php"); // must create $conn as PDO object
...
if (isset($_POST['draw'])) {
    ...
    // ✅ Total Records
    $totalStmt = $conn->query("SELECT COUNT(*) as total FROM temp");
    $totalResult = $totalStmt->fetch(PDO::FETCH_ASSOC);
    $totalData = $totalResult['total'];
    $totalFiltered = $totalData;

    // ✅ Base Query
    $sql = "SELECT * FROM temp WHERE 1=1";
    $params = array();
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        if (strtolower($row['Status']) == "processing") {
            $nestedData['action'] = "<a href='ccavenue_otherfees_test.php?process_id=" 
                                    . $row['tranID'] . 
...
<head>
    <meta charset="UTF-8">
    <title>Temp OtherFees Dashboard</title>
...
<div class="col-12 mt-4 p-4">
    <h3 class="mb-4">Temp OtherFees Records</h3>
...
$(document).ready(function () {
    $('#erpTable').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: "temp_otherfees_payment_tracker.php",
            type: "POST"
        },
        order: [[8, "desc"]],
        columns: [
