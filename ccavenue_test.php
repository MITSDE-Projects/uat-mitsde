                ...
                <script>
                    document.getElementById("pushPaymentBtn").addEventListener("click", function () {
                        if (confirm("Are you sure you want to push the payment?")) {
                            window.location.href = "temp_erp_response.php?oid=<?php echo urlencode($order_no); ?>&refno=<?php echo urlencode($reference_no); ?>&odt=<?php echo urlencode($order_date_time); ?>";
                        }
                    });
                </script>
                <?php
