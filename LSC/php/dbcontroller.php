...
class DBController{
	...
	*/
	public function dbregister($email, $pass, $flname, $hash, $country){
		...
		$q = "insert into users(pk_user,email,flname,password,usr_confirm_hash,number,usr_ip) values('NULL','$email','$flname','$pass','$hash','$country','$ip')";
    ...
    */
	...
	// suryakant
	public function dbgetUserAdmissionDetails($userkey){
		$q = "SELECT U.*,A.*,U.email as emailid FROM users U,student A WHERE U.pk_user = '$userkey' AND A.uid = U.pk_user";
	...
	// suryakant
	public function dbgetUserAdmissionDetails_print($userkey){
		$q = "SELECT U.*,A.* FROM users U,student A WHERE U.pk_user = '$userkey' AND A.uid = U.pk_user AND A.Is_Active = 1";
	...
	// icard count
	public function dbggetIdcardCount(){
		$q = "SELECT * FROM idcardcount";
	...
	//end
	// icard count update
	public function dbgetIdcardCountUpdate($rcount){
		$q = "UPDATE idcardcount SET icard_rollno = $rcount ";
	...
	//end
	// test center
	public function dbgetTest_Center($testcentre){
		$q = "select * from contact_address where testcenter = '$testcentre'";
    ...
    */
	...
	public function InsertAdmissionForm($userid,$ProgrammesUGPG,$Desciplines,$fname,$mname,$lname,$DateOfBirth,$Age,$PlaceofBirth,$Nationality,$upload_image,$Address,$City,$State,$PinCode,$PhoneNumber,$email,$AddressforCorrespondence,$cCity,$CState,$CPinCode,$CPhoneNumber,$CEmail,$ParentsName,$RelationshipwithApplicant,$Professionoftheparent,$AnnualIncome,$ParentMobileNumber,$ParentEmail,$BloodGroup,$PhysicallyChallenged,$AnyMajorillness,$Specifyillnes,$ExamBoardName10,$Percentage10,$YearofPassing10,$YearGap10,$ExamBoardName12,$Percentage12,$YearofPassing12,$YearGap12,$ExamDiploma,$PercentageDiploma,$YearofPassingDiploma,$YeargapDiploma,$CompanyName,$Experience,$testcenter,$colorRadio,$DDdate,$DDNumber,$BankName){
		$query = "insert into student(uid,programmesugpg,desciplines,name,middlename,lastname,dateofbirth,age,placeofbirth,nationality,photo_image,address,city,state,pincode,phonenumber,email,addressforcorrespondence,ccity,cstate,cpincode,cphonenumber,cemail,parentsname,relationshipwithapplicant,professionoftheparent,annualincome,parentmobilenumber,parentemail,bloodgroup,physicallychallenged,anymajorillness,specifyillnes,examboardname10,percentage10,yearofpassing10,yeargap10,examboardname12,percentage12,yearofpassing12,yeargap12,examdiploma,percentagediploma,yearofpassingdiploma,yeargapdiploma,companyname,experience,testcenter,colorRadio,dddate,ddnumber,bankname) values($userid,'$ProgrammesUGPG','$Desciplines','$fname','$mname','$lname','$DateOfBirth','$Age','$PlaceofBirth','$Nationality','$upload_image','$Address','$City','$State',$PinCode,'$PhoneNumber','$email','$AddressforCorrespondence','$cCity','$CState',$CPinCode,'$CPhoneNumber','$CEmail','$ParentsName','$RelationshipwithApplicant','$Professionoftheparent','$AnnualIncome','$ParentMobileNumber','$ParentEmail','$BloodGroup','$PhysicallyChallenged','$AnyMajorillness','$Specifyillnes','$ExamBoardName10','$Percentage10','$YearofPassing10','$YearGap10','$ExamBoardName12','$Percentage12','$YearofPassing12','$YearGap12','$ExamDiploma','$PercentageDiploma','$YearofPassingDiploma','$YeargapDiploma','$CompanyName','$Experience','$testcenter','$colorRadio','$DDdate','$DDNumber','$BankName')";
//echo $query;exit;
		$results = mysqli_query($this->link,$query);
		return $results ;
	}	    

	public function UpdateAdmissionForm($userid,$Address,$City,$State,$PinCode,$AddressforCorrespondence,$cCity,$CState,$CPinCode,$CPhoneNumber,$CEmail,$ParentsName,$RelationshipwithApplicant,$Professionoftheparent,$AnnualIncome,$ParentMobileNumber,$ParentEmail,$ExamBoardName10,$Percentage10,$YearofPassing10,$YearGap10,$ExamBoardName12,$Percentage12,$YearofPassing12,$YearGap12,$ExamDiploma,$PercentageDiploma,$YearofPassingDiploma,$YeargapDiploma,$CompanyName,$Experience,$testcenter){
		$query ="UPDATE student SET address = '$Address', city='$City', state='$State', pincode='$PinCode', addressforcorrespondence='$AddressforCorrespondence',ccity='$cCity',cstate='$CState', cpincode='$CPinCode',cphonenumber='$CPhoneNumber',cemail='$CEmail', parentsname='$ParentsName', relationshipwithapplicant='$RelationshipwithApplicant', professionoftheparent='$Professionoftheparent',annualincome='$AnnualIncome',parentmobilenumber='$ParentMobileNumber',parentemail='$ParentEmail',examboardname10='$ExamBoardName10',percentage10='$Percentage10',yearofpassing10='$YearofPassing10',yeargap10='$YearGap10',examboardname12='$ExamBoardName12',percentage12='$Percentage12',yearofpassing12='$YearofPassing12',yeargap12='$YearGap12',examdiploma='$ExamDiploma',percentagediploma='$PercentageDiploma',yearofpassingdiploma='$YearofPassingDiploma',yeargapdiploma='$YeargapDiploma',companyname='$CompanyName',experience='$Experience',testcenter='$testcenter' WHERE uid = $userid";
		
//echo $query;exit;
		$results = mysqli_query($this->link,$query);
		return $results ;
	}
	public function InsertPaymentHistory($userid,$paymentdate,$payment){
		 $query = "insert into payment_history(userid,paymentdate,payment) values($userid,'$paymentdate',$payment)";
//echo $query;exit;
		$results = mysqli_query($this->link,$query);
		return $results ;
	}		
	public function UpdateUserPaymentStatus($userid){
		$query ="UPDATE student SET isPayment = 1 WHERE uid = $userid";
