   ...
   //echo "</br>SELECT memberID,amount FROM student WHERE email='".$emailid."' and memberID='".$memberID."'";
    ...
    $getmarchanID = mysqli_query($conn,"SELECT memberID,amount,transactid FROM student WHERE email='".$emailid."' and memberID='".$memberID."'");
	            ...
	            if($amount==NULL)
	            {
	              ...
	              mysqli_query($conn,"UPDATE `student` SET amount='$inseramount', `transactid` = '$pay_id', `isPayment` = '1', `terms` = '1',`colorRadio`='NEFT',`Is_Active`='1', `paymenttype`='4',formstatus='payment done',lastPage='printformvalue.php',paydate='$pay_Date' WHERE `email` = '$emailid' AND memberID ='$Leadid'");
...
//die;
   mysqli_query($conn,"INSERT INTO `tbl_transactions_details` (`id`, `memberID`, `ERPLeadID`, `Name`, `Mobile_no`, `email`, `courseid`, `SpecializationID`, `FeeHeadID`, `fees_type`, `ins_1_amt`, `ins_2_amt`, `ins_3_amt`, `ins_1_date`, `ins_2_date`, `ins_3_date`, `ClearedDate`, `pay_type`, `payment_source`, `PayerBankID`, `transaction_id`, `order_id`, `UTR_number`, `payment_verification`, `PayeeInstituteID`, `PayeeBankID`, `PayeeACNo`, `PayeeACName`, `PayeeBranch`, `PayeeBankAddress`, `PayeeIFSCCode`, `UserId`, `CurrencyID`, `S_Flag`, `response`, `F_Flag`, `loanStatus`, `LoanProvider`, `API_DT`) VALUES (NULL, '".$Leadid."', NULL, NULL, NULL, '".$emailid."', NULL, NULL, '', NULL, '".$inseramount."', '0', '0', '".$pay_Date."', '0000-00-00', '0000-00-00', '', NULL, 'NEFT', NULL, '".$pay_id."', NULL, NULL, 'Not_Verified', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, '', '');");
