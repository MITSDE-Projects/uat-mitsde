...
<?php
                $body  = ob_get_clean();
                        ...
                        $mail->AddBCC('anees.shaikh@avantika.edu.in');
                         $mail->AddBCC('jyoti.deshmukh@avantika.edu.in');
                        $mail->Send();
 ...
 $getdata = mysqli_query($conn, "SELECT * FROM tbl_dat_result WHERE email='".$_GET['email']."'");
   while($setdata = mysqli_fetch_array($getdata)){
     ...
     GetSMSUrl($msg,$setdata['phone']);
      ...
      mysqli_query($conn,"UPDATE tbl_dat_result SET scheduled_email_again='1' WHERE id='".$setdata['id']."'");
