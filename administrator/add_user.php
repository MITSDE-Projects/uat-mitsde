...
/*
 ...
 $getdata = mysqli_query($conn,"SELECT * FROM tbl_wildcardentry");
  while($setdata = mysqli_fetch_array($getdata)){
  
   mysqli_query($conn,"INSERT INTO tbl_results_studio(roll_no,name,message,studio_flag)VALUES('".$setdata['roll_no']."','".$setdata['name']."','Congratulations! You have been selected for B.Des Program at Avantika University, Ujjain.','1')");
  
  }
 exit;
 
*/
$getlogdtls = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM adminlogin WHERE id='".$_GET['id']."'"));


if(isset($_GET['action']) && $_GET['action']=='deleteimg')
{
	$delimgsuc = mysqli_query($conn,"UPDATE adminlogin SET profile_pic='' WHERE id='".$_GET['id']."'");	
	unlink("profile_pic/".$_GET['val']);
	if($delimgsuc){
		header('location:add_user.php?msg=image_deleted');
	}
}
...
if(isset($_POST['submit']))
{
	...
	$addquery = mysqli_query($conn,"INSERT INTO adminlogin(full_name,username,password,phone_num,profile_pic)VALUES('".$full_name."','".$username."','".$password."','".$phone_num."','".$profile_pic."')");
	$lstinsid = mysqli_insert_id($conn);
	 mysqli_query($conn,"INSERT INTO tbl_access_rights(user_id)VALUES('".$lstinsid."')");
	if($addquery){
		
		header('location:list_users.php?msg=user_added');
	}
...
if(isset($_POST['update']))
{
	...
	$updatequery = mysqli_query($conn,"UPDATE adminlogin SET full_name='".$full_name."',username='".$username."',password='".$password."',phone_num='".$phone_num."',profile_pic='".$profile_pic."' WHERE id='".$_GET['id']."'");
...
function validations()
{
	...
	var  full_name = document.getElementById('full_name').value;
	 if(full_name==''){
		document.getElementById('full_name_err').innerHTML='Enter Full Name';
              ...
              <div class="box box-primary">
                ...
                <form role="form" method="POST" enctype="multipart/form-data" onsubmit="return validations();">
                  <div class="box-body">
                    ...
                    <input type="text" class="form-control" name="full_name" value="<?=$getlogdtls['full_name']?>" id="full_name" placeholder="Enter Full Name">
					<div id="full_name_err" class="class_err"></div>
                    </div>
				  
				  
                    <div class="form-group">
                      <label for="exampleInputEmail1">Email address</label>
                    <input type="text" class="form-control" name="username" value="<?=$getlogdtls['username']?>" id="username" placeholder="Enter Username">
