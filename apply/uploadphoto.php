...
if (isset($_POST['submiteventimg']) && $_POST['submiteventimg']=="Submit") {
 ...
 if (strlen($name)) {
   ...
   $img_date_time= date('Y-m-d h:i:s');

     if(mysqli_query($connection,"UPDATE tbl_students_data SET photo='".$filepath.'/'.$filename."',`dataTime` = '$img_date_time' WHERE student_id='".$memberID."'"))
     {
