...
if(isset($_POST["dpt"]) && !empty($_POST["dpt"])){
    ...
    echo "</br>dpt-->".$dptid=$_POST["dpt"];
	 ...
	 //echo "SELECT * FROM SpecializationERP WHERE CourseID = '".$dptid."'"; 
	 //exit; 
    ...
    if($rowCount > 0)
	{
		echo '<option value="">Course List</option>';
        while($row = mysqli_fetch_array($getquery))
        { 
            echo '<option value="'.$row['SpecializationID'].'_'.$row['SpecializationName'].'">'.$row['SpecializationID'].'_'.$row['SpecializationName'].'</option>';
		}
    } 
    else 
    {
