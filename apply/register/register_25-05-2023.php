...
echo "</br>MobileNumber-->".$MobileNumber=$_POST['phonenumber'];
...
//echo  "</br>leadid--->".$leadid=$resp[string][userId]; 
   ...
   if($leadId!=0)
   {
      echo "</br>ID-->". $IP=$_SERVER['REMOTE_ADDR']; 
       echo "</br>userid-->".$leadId;
...
?>        
   ...
   else
   {
	...
	if(!filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)){
		...
		if(!empty($row['email'])){
			$error[] = 'Email provided is already in use.';
			header('Location:https://www.mitsde.com/apply/register/register.php?action=exitmailid');
		}
...
<input type="checkbox" required style="" id="checkbox-pos"><div style="" id="div-terms-cond"><a href="#" target="_blank" id="termcondi">I authorize MIT-SDE representative to contact me,this will override DND/NDNC registry</a></div>
              <br>
            	<div><button type="submit" name="register" id="register" disabled="disabled" value="Register" style="background:#606062;color:#FFF;width:99px;font-size: 14px;padding:5px 10px;" onsubmit = "validate('menuContactform1')">Register</button></div>
            </form>
