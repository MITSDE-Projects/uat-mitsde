...
//echo  "</br>leadid--->".$leadid=$resp[string][userId]; 
   ...
   if($leadId!=0)
   {
       ...
       if(strlen($_POST['email']) < 5)
	{
		...
		if(!empty($row['email']))
		{
		    header('Location:https://www.mitsde.com/apply/register/index1.php?action=exitmailid&firstname='.$name.'&lastname='.$lastname.'&emailid='.$Email.'&mob='.$MobileNumber.'');
...
?>        
   ...
   else
   {
       ...
       if(strlen($_POST['email']) < 5)
	{
		...
		if(!empty($row['email']))
		{
		    header('Location:https://www.mitsde.com/apply/register/index.php?action=exitmailid');
			//$error[] = 'Email provided is already in use.';
		}
...
///echo $stmt->insert_id; exit;
          ...
          //-----------------------------------------------------------------SEND Mail Function with verifi extra edge--------------------------	
		   /*$mail  = new PHPMailer();
           ob_start(); //Turn on output buffering
           $email =  $_POST['email'];
?>        
   ...
   <div class="formcontainer">
       ...
       <h3 style="float:right;">New User? Register</h3>
			...
			<input name="email" type="email" id="email" style="margin-top:0px;font-size: 14px;" required  placeholder="Email Address" value="<?=$_POST['email']?>">
			 ...
			 <div><label id="label-password" style="left: 80%;">Captcha <sanp style="color:red;">*</sanp></label></div>
			 <div class="row" >
...
<input type="checkbox" required style="" id="checkbox-pos"><div style="" id="div-terms-cond"><a href="#" target="_blank" id="termcondi">I authorize MIT-SDE representative to contact me,this will override DND/NDNC registry</a></div>
              <br>
            	<div><button type="submit" name="register" id="register"  value="Register" style="background:#606062;color:#FFF;width:99px;font-size: 14px;padding:5px 10px;">Register</button></div>
            </form>
