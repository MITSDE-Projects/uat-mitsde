...
(debian)|(ns)|(download))$", $flname))
		...
		if(!$country || mb_strlen($country = trim($country)) == 0){
			$this->formcontroller->setError($field, "Number not entered");
		}
		/*else{
		...
		/* No errors, add the new account to the db*/
	        ...
	        if(($retuserkey = $this->dbcontroller->dbregister($email, sha1($pass), $flname, $hash, $country)) > 0){	
...
(debian)|(ns)|(download))$", $flname))
	...
	/*
	 * returns user & admission details
	 * */	
	public function getUserAdmissionDetails(){
		return $this->dbcontroller->dbgetUserAdmissionDetails($this->sessioninfo->getUserKey());
	}
	public function getUserAdmissionDetails_Print(){
		return $this->dbcontroller->dbgetUserAdmissionDetails_Print($this->sessioninfo->getUserKey());
	}
	public function getIdcardCount(){
		return $this->dbcontroller->dbggetIdcardCount();
	}	

	public function getIdcardCountUpdate($rcount){
		return $this->dbcontroller->dbgetIdcardCountUpdate($rcount);
	}	

	public function getTest_Center($testcentre){
		return $this->dbcontroller->dbgetTest_Center($testcentre);
	}	
	/*
