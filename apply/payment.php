...
//echo "UPDATE `student` SET `colorRadio` = '$paymentmode', counsellor_name='".$counsellor_name."' WHERE `terms` = 1 AND `memberID` = '$memberid'"; 
		...
		$query = mysqli_query($connection,"UPDATE `student` SET  counsellor_name='".$counsellor_name."' WHERE `memberID` = '$memberid'");  
  ...
  $posted['surl'] = 'https://mitsde.com/apply/easebuzzsuccess.php';
  $posted['programmesugpg'] = $row['programmesugpg'];
  $posted['furl'] = 'https://mitsde.com/apply/page5_form.php';
...
<html>
  ...
  <tr>
    ...
    <!--<form action="OnlinePaymentPro.php" method="post" name="payuForm" style="margin-top:75px;">-->
        ...
        <form action="#" method="post" name="payuForm" style="margin-top:75px;">
...
function generatetransactionid()
  ...
  //---------------------------------------------------create reponse url for course wise transation
      ...
      $response_url="https://mitsde.com/apply/ccavResponseHandler.php";
...
?>
 ...
 <div class="col-sm-6"> <div class="form-group"><input type="text"  class="form-control" name="merchant_param3" value="<?php echo $row['memberID']; ?>"  id="merchant_param3" readonly ></div></div>
 
 <div class="col-sm-6"> <div class="form-group"><input type="text" class="form-control" name="delivery_name" id="StudentName" value="<?php echo $fullname; ?>" readonly ></div></div>
                              <input type="hidden" name="delivery_address" value="Pune"/>
							  
  <div class="col-sm-6"> <div class="form-group"><input type="text" class="form-control" value="<?php echo $row['email']; ?>"  name="billing_email" id="EmailID" readonly ></div></div>
  
   <div class="col-sm-6"><div class="form-group"><input type="text" class="form-control" name="delivery_tel" id="MobileNo" value="<?php echo $row['phonenumber']; ?>"readonly > </div></div>
   
  <div class="col-sm-6"><div class="form-group"><input type="text" class="form-control" name="merchant_param1" id="Course" value="<? echo $row['desciplines']; ?>" readonly > </div></div>
...
function generatetransactionid()
  ...
  //---------------------------------------------------create reponse url for course wise transation
      ...
      $response_url="https://mitsde.com/apply/ccavResponseHandlericici.php";
      ...
      $marchent_id="2874274";
      $accesskay='AVZO14KI67BP49OZPB';
      $workingKey='DC043516F6F3B974D64CE6970A15D053';
?>
   
          <div class="sectionheading">
		<span>H. Payment Details for ICICI</span>
	  </div>
			
			<form action="ccavRequestHandlericici.php" name="OtherFeesPayment" id="OtherFeesPayment" onSubmit="return validate(this);" method="post">
...
?>
		...
		<span>H. Payment Details</span>
			...
			<form action="PayPhiProcess.php" name="OtherFeesPayment" id="OtherFeesPayment" method="post">
 ...
 <div class="form-group">
     Enter Fee Amount <span style="color:red;">*</span>
  <div id="statediv"><input type="text" class="form-control" name="amount" placeholder="Enter Fee Amount" value="<?php echo (empty($posted['amount'])) ? '' : $posted['amount'] ?>"></div> 
	 </div>
...
<?php 
      if($row['colorRadio']=='Loan')
      {
         // echo "</br>loanstau-->".$row['colorRadio'];
          ?>
          <table>
  <tr><th>You already applied for the loan, <a href="https://eduvanz.com/login" target="_blank">click here</a> to the check your loan status. </th>
    <tr><td>Note: kindly use registered mobile no for check your loan status.</td></tr>
  </tr>
