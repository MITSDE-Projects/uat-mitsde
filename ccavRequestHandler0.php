<p align="center">MIT-SDE Redirect To HDFC Payment Gateway Please Wait Five Minutes</p>
...
$required = ['merchant_param3','delivery_name','billing_email','delivery_tel','merchant_param1','SpecializationID','merchant_param2','amount','order_id'];
...
$FeesType      = trim($_POST['merchant_param2']);
$totamt        = (int) $_POST['amount'];
$transactionId = trim($_POST['order_id']);

if (!$Email) die("Invalid email.");
if (strlen($MobileNo) < 10) die("Invalid mobile number.");


// =====================================================
// 5. DUPLICATE TRANSACTION CHECK (PDO)
// =====================================================
$stmt = $conn->prepare("SELECT 1 FROM OtherFeesTransactionN WHERE t_process_id = ? LIMIT 1");
