...
<body>
       ...
       if($order_status==="Success")
	{
		...
		/*  $transation = mysql_query("select PayU_ID,t_process_id,amount from OtherFeesTransaction where `leadID`='".$LeadID[1]."'");
			...
			$orchk = mysql_num_rows(mysql_query("select * from OtherFeesTransaction where `t_process_id`='".$oderID[1]."'"));
		...
		if($orchk>0 || $trchk >0 )
		  ...
		  elseif($temp1['tranID'] !=$oderID[1] || $temp1['T_B_Amount'] != $dotamt[0] )
		     ...
		     echo "<br><h3>Thank You for Payment.</h3>.";
		            $tdate= date("Y/m/d");
	                  ...
	                  $curdate = date('Y-m-d');
	                    ...
	                    //echo "</br>INSERT INTO `OtherFeesTransaction` (`othr_id`, `leadID`, `name`, `email`, `phone`, `CourseName`,`SpecializationID`, `FeeHeadID`, `FeesType`, `ReceiptType`, `amount`, `PayU_ID`, `payment_source`, `PayerBankID`, `transationDate`, `t_process_id`, `UTRNO`, `payment_confirmation_status`, `PayeeInstituteID`, `PayeeBankID`, `PayeeACNo`, `PayeeACName`, `PayeeBranch`, `PayeeBankAddress`, `PayeeIFSCCode`, `S_Flag`, `response`, `F_Flag`,`harbouroffering`,`learnerstatus`) VALUES (NULL, '".$LeadID[1]."', '".$temp1['student_name']."', '".$temp1['email_id']."','".$temp1['phone']."', '".$temp1['course']."','".$temp1['SpecializationID']."', '141', 'KPMG Backlog Fee','".$ReceiptType."','".$dotamt[0]."','".$transationID[1]."', 'ICICI','1', '".$tdate."','".$oderID[1]."', NULL, 'Not Verify', '16', '1', '50100267576292', 'Pune', 'Mayur Colony Kothrud Pune name', 'Mayur Colony Kothrud Pune address', 'HDFC0000149', '0', NULL, '1','".$temp1['fees_type']."','".$learnerstatus[1]."')";
			 ...
			 //$insertO_tras= "INSERT INTO `updpayment` (`id`, `LeadID`, `ReceiptType`, `PaymentType`, `PaymentModeID`, `InstruNo`, `InstruDate`, `ClearedDate`, `PaidAmount`, `FeeHeadID`, `PayerBankID`, `PayerBranch`, `PayerBankAddress`, `PayerIFSCcode`, `PayeeInstituteID`, `PayeeBankID`, `PayeeACNo`, `PayeeACName`, `PayeeBranch`,`PayeeBankAddress`, `PayeeIFSCCode`, `UserId`, `CurrencyID`) VALUES (NULL, '".$LeadID[1]."', 'OC', 'L','10', '".$transationID[1]."', '".$curdate."', '".$curdate."', '".".$dotamt[0]."."', '59', '10', NULL, NULL, NULL, '16', '1', '50100267576292', 'Pune','Mayur Colony Kothrud Pune name', 'Mayur Colony Kothrud Pune address', 'HDFC0000149', '8', '1')";
		          ...
		          $StudentName=explode('=',$decryptValues[19]);
		            ...
		            echo '<tr><td>Fees Type :</td><td>KPMG BA/BI Certification Course 3rd Attempt fee Payment</td></tr>';
				...
				<p>Thank you and see you soon.<br>
				 ...
				 <?php
		           /*$body  = ob_get_clean();
						...
						$mail->Subject    = "KPMG BA/BI Certification Course 3rd Attempt fee Payment Made Successfully";
						...
						$mail->AddBCC('monika.pateriya@mitsde.com');
		  ...
		  //------------------------------Success Mail END----------------------------------------
					...
					$emailquery = mysql_query("INSERT INTO email_tracker (id, emailid, pagename, email_status, data_time) VALUES (NULL, '" . $StudentEmailID[1] . "', 'KPMGPayment', 'success', '" . $DT . "')");
			    ...
			    <p>Thank you and see you soon.<br>
				 ...
				 <?php
		           $body  = ob_get_clean();
						...
						$mail->Subject    = "Canceled This Transaction for KPMG BA/BI Certification Course 3rd Attempt fee Payment";
						...
						$mail->Send();
		  //------------------------------Aborted Mail END----------------------------------------
		$emailquery = mysql_query("INSERT INTO email_tracker (id, emailid, pagename, email_status, data_time) VALUES (NULL, '" . $StudentEmailID[1] . "', 'KPMGPayment', 'abort', '" . $DT . "')");
		//---------------------------------------FOR DELETE FORM TEMP TABLE----------------------
		$deletetemp=mysql_query("DELETE FROM `temp` WHERE T_LeadID='".$LeadID[1]."' and tranID='".$oderID[1]."'");
		//---------------------------------------FOR DELETE FORM TEMP TABLE------END----------------
			    ...
			    <p>Thank you and see you soon.<br>
				 ...
				 <?php
		           $body  = ob_get_clean();
						...
						$mail->Subject    = "Current Transaction is Failed for KPMG BA/BI Certification Course 3rd Attempt fee Payment";
						...
						$mail->Send();
		  //------------------------------Failure Mail END----------------------------------------
		$emailquery = mysql_query("INSERT INTO email_tracker (id, emailid, pagename, email_status, data_time) VALUES (NULL, '" . $StudentEmailID[1] . "', 'KPMGPayment', 'failure', '" . $DT . "')");
		//---------------------------------------FOR DELETE FORM TEMP TABLE----------------------
