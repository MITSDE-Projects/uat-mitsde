		...
		<div class="container">
			<div class="row">
				...
				if ($order_status === "Success") {
					...
					$stmt = $conn->prepare("SELECT COUNT(*) FROM OtherFeesTransaction WHERE PayU_ID = ?");
					$stmt->execute([$transationID[1]]);
					$trchk = $stmt->fetchColumn();

					// Check for duplicate order ID
					$stmt = $conn->prepare("SELECT COUNT(*) FROM OtherFeesTransaction WHERE t_process_id = ?");
					...
					if ($orchk > 0 || $trchk > 0) {
						...
						try {
							...
							$stmt = $conn->prepare("INSERT INTO OtherFeesTransaction
								(leadID,name,email,phone,CourseName,SpecializationID,FeeHeadID,FeesType,ReceiptType,amount,PayU_ID,payment_source,PayerBankID,transationDate,t_process_id,payment_confirmation_status,PayeeInstituteID,PayeeBankID,PayeeACNo,PayeeACName,PayeeBranch,PayeeBankAddress,PayeeIFSCCode,S_Flag,response,F_Flag,harbouroffering,learnerstatus,API_DT,course_id,json_rs_payment)
								VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
